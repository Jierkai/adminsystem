    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <!-- 放置上传图片 -->
          <ImageUpload ref="PersonPhoto" :limit-num="3" />
        </el-form-item>
      ...
      <div class="block">
        ...
        <el-form-item label="入学时间">
          <el-date-picker v-model="formData.enrolmentTime" class="inputW" placeholder="请输入时间" type="data" />
        </el-form-item>
        <el-form-item label="毕业时间">
          <el-date-picker v-model="formData.graduationTime" class="inputW" placeholder="请输入时间" type="data" />
        </el-form-item>
...
export default {
  ...
  methods: {
    async getEmployeeDetail() {
      const res = await reqEmployeePersonInfo(this.userId)
      this.formData = { ...res.data }

      this.$refs.PersonPhoto.fileList = res.data.staffPhoto.split(',').map(i => {
        return { url: i }
      })
    },
    async getUserDetailById() {
      const res = await reqEmployeeDetail(this.userId)
    ...
    async saveUser() {
      const uploadRef = this.$refs.staffPhoto
      if (!uploadRef.isSuccess) {
        this.$message.error('当前有文件未上传成功')
        return
      }
      if (!uploadRef.fileList[0]) {
        this.$message.error('请上传员工头像')
        return
      }
      await updateEmployeeDetail(this.userId, {
        ...this.userInfo,
        staffPhoto: uploadRef.fileList[0].url
      })
      this.$message.success('保存成功')
    },
    async savePersonal() {
      const person = this.$refs.PersonPhoto
      const arrStr = person.fileList.map(i => i.url).join(',')
      if (!person.isSuccess) {
        this.$message.error('有文件未上传成功')
        return
      }
      if (!person.fileList[0]) {
        this.$message.error('请上传员工照片')
      }
      await updateEmployeePersonInfo(
        this.userId,
        {
          ...this.formData,
          staffPhoto: arrStr
