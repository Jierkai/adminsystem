      ...
      <el-card>
        ...
        <div v-if="type === 'personal'">
          ...
          <table cellspacing="0" class="tableList" width="100%">
            ...
            <tr>
              <th>出生日期</th>
              <td colspan="6">{{ formData.dateOfBirth | timeFilter }}</td>
            </tr>
            <tr>
              <th>最高学历</th>
...
<script>
import { reqEmployeeDetail, reqEmployeeJobDetail, reqEmployeePersonInfo } from '@/api/employee'
...
export default {
  ...
  methods: {
    async getPersonalDetail() {
      const p1 = await reqEmployeePersonInfo(this.userId)
      const p2 = await reqEmployeeDetail(this.userId)
      this.formData = { ...p1.data, ...p2.data }
    },
    async getJobDetail() {
      const job = await reqEmployeeJobDetail(this.userId)
      this.formData = { ...job.data }
    }
  }
}
</script>
